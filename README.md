...
Spring Security의 자동 OAuth2 Login을 사용하지 않고, `RestClientAuthorizationCodeTokenResponseClient`를 직접 활용하여 code → token 교환을 수동 구현했습니다. `ClientRegistration`과 `OAuth2AuthorizationExchange`를 프로그래밍 방식으로 구성합니다.

인가 서버 JWT 검증
인가 서버는 JWT를 RS256(비대칭키) 알고리즘으로 서명합니다. 리소스 서버는 인가 서버의 [JWKS(JSON Web Key Set)](https://datatracker.ietf.org/doc/html/rfc7517) 엔드포인트(/oauth2/jwks)에서 공개키를 가져와 서명을 검증합니다. 키가 교체(rotation)되더라도 엔드포인트에서 새 키를 자동으로 가져오므로 수동 개입이 필요 없습니다.
OAuth2ClientConfig에서 NimbusJwtDecoder를 JWKS 엔드포인트로 구성하여 Bean으로 등록하며, OAuth2LoginSuccessHandler에서 인가 서버 Access Token을 디코딩할 때 사용됩니다. spring.security.oauth2.resourceserver 프로퍼티는 설정하지 않았기 때문에 Spring Boot의 Resource Server 자동 구성(자동 SecurityFilterChain 생성)은 비활성화된 상태이며, JWT 검증 로직을 직접 제어합니다.

### Stateless 인증

`JwtAuthenticationFilter`는 `OncePerRequestFilter`를 상속하여 매 요청마다 자체 JWT를 검증하고, `SecurityContextHolder`에 인증 정보를 저장합니다. 세션을 사용하지 않는 완전한 Stateless 구조입니다.